...
export async function POST(request: Request) {
  try {
    ...
    const education = await prisma.uGEducation.create({
      data: {
        candidateId,
        courseName,
    ...
    console.error('Error adding UG education:', error);
    return NextResponse.json({ error: 'Failed to add UG education' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'UG education ID required' }, { status: 400 });
    }

    await prisma.uGEducation.delete({
      where: { id }
    });

    return NextResponse.json({ success: true }, { status: 200 });
  } catch (error) {
    console.error('Error deleting UG education:', error);
    return NextResponse.json({ error: 'Failed to delete UG education' }, { status: 500 });
  }
}
